...
/// Check if the user has accepted the first-run disclaimer.
/// If not, display it and ask for confirmation.
fn check_first_run_disclaimer() {
    let disclaimer_path = dirs::config_dir()
        .unwrap_or_else(|| {
            let home = std::env::var("HOME").unwrap_or_else(|_| ".".to_string());
            PathBuf::from(home).join(".config")
        })
        .join("null-e")
        .join(".disclaimer-accepted");
    ...
    eprintln!(
        "{}\n",
        format!("{}  DISCLAIMER: null-e deletes files from your disk.", "⚠")
            .yellow()
            .bold()
    );
    eprintln!(
        "  {} Files are moved to trash by default (--method trash)",
        "•".dimmed()
    );
    eprintln!(
    ...
    eprintln!("  {} No warranty — use at your own risk", "•".dimmed());
    eprintln!("  {} Always keep backups of important work", "•".dimmed());
    eprintln!();
    eprint!("  Continue? [Y/n] ");
    io::stderr().flush().ok();
    ...
    std::fs::write(&disclaimer_path, "accepted\n").ok();
}
...
fn main() {
    ...
    // Warn when using force + permanent delete
    if cli.force && cli.method == DeleteMethodArg::Permanent {
        eprintln!(
            "{}",
            format!(
                "{}  Force mode + permanent delete: files will be irrecoverably removed.",
                "⚠"
            )
            .yellow()
            .bold()
...
fn cmd_scan(cli: &Cli, detailed: bool) -> Result<()> {
    ...
    let mut cache = if use_cache {
        null_e_core::cache::load_cache().unwrap_or_default()
    } else {
        null_e_core::cache::ScanCache::new()
    };
    ...
    if use_cache {
        ...
        cache.touch();
        let _ = null_e_core::cache::save_cache(&cache); // Ignore save errors
    }
...
fn cmd_clean(cli: &Cli, _only: &[String], _exclude: &[String]) -> Result<()> {
    ...
    null_e_core::git::enrich_with_git_status(&mut projects)?;
    ...
    let (cleanable, blocked): (Vec<_>, Vec<_>) = projects.into_iter().partition(|p| {
        let check = null_e_core::git::check_project_protection(p, protection_level);
...
fn cmd_config(init: bool, show_path: bool) -> Result<()> {
    if init {
        let path = null_e_core::config::init_config()?;
    ...
    if show_path {
        let path = null_e_core::config::default_config_path()?;
        println!("{}", path.display());
        return Ok(());
    }

    // Show current config
    let config = null_e_core::config::load_default_config()?;
...
fn cmd_caches(cli: &Cli, clean: bool, clean_all: bool, use_official: bool) -> Result<()> {
    use null_e_core::caches::{calculate_all_sizes, detect_caches, CachesSummary};
...
fn clean_selected_caches(
    caches: &[null_e_core::caches::GlobalCache],
    cli: &Cli,
    use_official: bool,
) -> Result<()> {
    use null_e_core::caches::clean_cache;
...
fn cmd_sweep(cli: &Cli, clean: bool, category: Option<&str>) -> Result<()> {
    ...
    for (i, item) in all_items.iter().take(display_count).enumerate() {
        ...
        let safety = match item.safe_to_delete {
            null_e_core::cleaners::SafetyLevel::Safe => "✓ Safe".green().to_string(),
            null_e_core::cleaners::SafetyLevel::SafeWithCost => "~ Rebuild".yellow().to_string(),
            null_e_core::cleaners::SafetyLevel::Caution => "! Caution".red().to_string(),
            null_e_core::cleaners::SafetyLevel::Dangerous => "⚠ Danger".magenta().to_string(),
        };
...
fn clean_items_interactive(
    items: &[null_e_core::cleaners::CleanableItem],
    cli: &Cli,
) -> Result<()> {
    println!(
        ...
        if cli.dry_run {
            ...
            if let Some(ref cmd) = item.clean_command {
                ...
                if run_clean_command_silent(cmd).is_ok() {
                    ...
                    match null_e_core::trash::delete_path(&item.path, delete_method) {
                        Ok(_) => {
                            cleaned_size += item.size;
                            cleaned_count += 1;
                        }
...
fn cmd_xcode(cli: &Cli, clean: bool) -> Result<()> {
    use null_e_core::cleaners::xcode::XcodeCleaner;
...
fn cmd_android(cli: &Cli, clean: bool) -> Result<()> {
    use null_e_core::cleaners::android::AndroidCleaner;
...
fn cmd_docker(cli: &Cli, clean: bool, include_volumes: bool) -> Result<()> {
    use null_e_core::cleaners::docker::DockerCleaner;
...
fn cmd_ml(cli: &Cli, clean: bool) -> Result<()> {
    use null_e_core::cleaners::ml::MlCleaner;
...
fn cmd_ide(cli: &Cli, clean: bool) -> Result<()> {
    use null_e_core::cleaners::ide::IdeCleaner;
...
fn cmd_homebrew(cli: &Cli, clean: bool, scrub: bool) -> Result<()> {
    use null_e_core::cleaners::homebrew::HomebrewCleaner;
...
fn cmd_ios_deps(cli: &Cli, clean: bool) -> Result<()> {
    use null_e_core::cleaners::ios_deps::IosDependencyCleaner;
...
fn cmd_electron(cli: &Cli, clean: bool) -> Result<()> {
    use null_e_core::cleaners::electron::ElectronCleaner;
...
fn cmd_gamedev(cli: &Cli, clean: bool) -> Result<()> {
    use null_e_core::cleaners::gamedev::GameDevCleaner;
...
fn cmd_cloud(cli: &Cli, clean: bool) -> Result<()> {
    use null_e_core::cleaners::cloud::CloudCliCleaner;
...
fn cmd_macos(cli: &Cli, clean: bool) -> Result<()> {
    use null_e_core::cleaners::macos::MacOsCleaner;
...
fn cmd_git_analyze(cli: &Cli, fix: bool) -> Result<()> {
    use null_e_core::analysis::git::GitAnalyzer;
...
fn cmd_stale(cli: &Cli, days: u64, clean: bool) -> Result<()> {
    use null_e_core::analysis::stale::StaleProjectFinder;
    ...
    for (i, rec) in all_recommendations.iter().enumerate() {
        ...
        let risk_color = match rec.risk {
            null_e_core::analysis::RiskLevel::None | null_e_core::analysis::RiskLevel::Low => {
                cleanable.green().to_string()
            }
            null_e_core::analysis::RiskLevel::Medium => cleanable.yellow().to_string(),
            null_e_core::analysis::RiskLevel::High => cleanable.red().to_string(),
        };
...
fn cmd_duplicates(cli: &Cli) -> Result<()> {
    use null_e_core::analysis::duplicates::DuplicateFinder;
