...
use null_e_core::core::{Project, ScanConfig, ScanResult, Scanner};
...
impl App {
    ...
    fn scan_all(tx: Sender<ScanMessage>, paths: Vec<PathBuf>) {
        ...
        if let Ok(mut caches) = null_e_core::caches::detect_caches() {
            let _ = null_e_core::caches::calculate_all_sizes(&mut caches);
        ...
        if let Some(cleaner) = null_e_core::cleaners::xcode::XcodeCleaner::new() {
        ...
        if let Some(cleaner) = null_e_core::cleaners::ide::IdeCleaner::new() {
            if let Ok(items) = cleaner.detect() {
                for item in items {
                    all_cleaners.push(CleanerEntry {
                        name: item.name,
        ...
        if let Some(cleaner) = null_e_core::cleaners::ml::MlCleaner::new() {
            if let Ok(items) = cleaner.detect() {
                for item in items {
                    all_cleaners.push(CleanerEntry {
                        name: item.name,
        ...
        if let Some(cleaner) = null_e_core::cleaners::android::AndroidCleaner::new() {
            if let Ok(items) = cleaner.detect() {
                for item in items {
                    all_cleaners.push(CleanerEntry {
                        name: item.name,
        ...
        if let Some(cleaner) = null_e_core::cleaners::electron::ElectronCleaner::new() {
            if let Ok(items) = cleaner.detect() {
                for item in items {
                    all_cleaners.push(CleanerEntry {
                        name: item.name,
        ...
        if let Some(cleaner) = null_e_core::cleaners::cloud::CloudCliCleaner::new() {
            if let Ok(items) = cleaner.detect() {
                for item in items {
                    all_cleaners.push(CleanerEntry {
                        name: item.name,
        ...
        if let Some(cleaner) = null_e_core::cleaners::homebrew::HomebrewCleaner::new() {
            if let Ok(items) = cleaner.detect() {
                for item in items {
                    all_cleaners.push(CleanerEntry {
                        name: item.name,
        ...
        if let Some(cleaner) = null_e_core::cleaners::gamedev::GameDevCleaner::new() {
            if let Ok(items) = cleaner.detect() {
                for item in items {
                    all_cleaners.push(CleanerEntry {
                        name: item.name,
        ...
        if let Some(cleaner) = null_e_core::cleaners::misc::MiscCleaner::new() {
            if let Ok(items) = cleaner.detect() {
                for item in items {
                    all_cleaners.push(CleanerEntry {
                        name: item.name,
        ...
        if let Some(cleaner) = null_e_core::cleaners::browsers_test::TestBrowsersCleaner::new() {
            if let Ok(items) = cleaner.detect() {
                for item in items {
                    all_cleaners.push(CleanerEntry {
                        name: item.name,
        ...
        if let Some(cleaner) = null_e_core::cleaners::system::SystemCleaner::new() {
            if let Ok(items) = cleaner.detect() {
                for item in items {
                    all_cleaners.push(CleanerEntry {
                        name: item.name,
        ...
        if let Some(cleaner) = null_e_core::cleaners::logs::LogsCleaner::new() {
            if let Ok(items) = cleaner.detect() {
                for item in items {
                    all_cleaners.push(CleanerEntry {
                        name: item.name,
        ...
        if let Some(cleaner) = null_e_core::cleaners::runtimes::RuntimesCleaner::new() {
            if let Ok(items) = cleaner.detect() {
                for item in items {
                    all_cleaners.push(CleanerEntry {
                        name: item.name,
        ...
        if let Some(analyzer) = null_e_core::cleaners::binaries::BinaryAnalyzer::new() {
    ...
    fn scan_caches(tx: Sender<ScanMessage>) {
        ...
        match null_e_core::caches::detect_caches() {
            Ok(mut caches) => {
                let _ = null_e_core::caches::calculate_all_sizes(&mut caches);
    ...
    fn scan_docker(tx: Sender<ScanMessage>) {
        ...
        let cleaner = null_e_core::cleaners::docker::DockerCleaner::new();
