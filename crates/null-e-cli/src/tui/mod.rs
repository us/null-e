...
fn run_app(
    ...
    loop {
        ...
        let event = events
            .next()
            .map_err(|e| null_e_core::error::NullEError::Other(e.to_string()))?;
        match event {
            Event::Key(key) => {
...
fn run_clean_command(cmd: &str) -> null_e_core::error::Result<()> {
    ...
    match output {
        ...
        Ok(out) => Err(null_e_core::error::NullEError::Other(
            String::from_utf8_lossy(&out.stderr).to_string(),
        )),
        Err(e) => Err(null_e_core::error::NullEError::Other(e.to_string())),
    }
}
